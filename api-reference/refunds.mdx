...
<ParamField query="reference" type="string" required>
  The transaction reference (e.g., `TXN_gGbEEvK25eVgmgrf` or your custom reference)
</ParamField>

```bash
curl -X GET "https://api.rollapay.com/transaction/refundable?reference=TXN_gGbEEvK25eVgmgrf" \
...
<Card title="POST /transaction/refund" icon="rotate-left">
...
curl -X POST "https://api.rollapay.com/transaction/refund" \
  -H "Authorization: Bearer sk_live_your_secret_key" \
...
```javascript Node.js
const response = await fetch('https://api.rollapay.com/transaction/refund', {
  method: 'POST',
  headers: {
    'Authorization': 'Bearer sk_live_your_secret_key',
...
response = requests.post(
    'https://api.rollapay.com/transaction/refund',
    headers={
        'Authorization': 'Bearer sk_live_your_secret_key',
        'Content-Type': 'application/json',
    },
...
{/* List Refunds - Commented out for now

## List Refunds

Retrieve a paginated list of refunds for your organization.

<Card title="GET /refunds" icon="list">
...
<RequestExample>
```bash cURL
curl -X GET "https://api.rollapay.com/refunds&status=completed&limit=10" \
  -H "Authorization: Bearer sk_live_your_secret_key"
```
</RequestExample>
...
<Card title="GET /refunds/:id" icon="magnifying-glass">
  Get refund by ID
</Card>
...
<RequestExample>
```bash cURL
curl -X GET "https://api.rollapay.com/refunds/{refundId}" \
  -H "Authorization: Bearer sk_live_your_secret_key"
```
</RequestExample>
...
{/* Self-Service Limits - Commented out for now
