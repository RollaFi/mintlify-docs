...
<CardGroup cols={2}>
  ...
  <Card title="Mobile Money" icon="mobile-screen" href="/api-reference/initiate-payout">
    Send funds to a mobile money wallet. Available for GHS and XOF.
  </Card>
</CardGroup>
...
Both events are enabled by default. You can turn them off under **Settings > API Keys & Webhooks** in the [Rolla Dashboard](https://merchant.rollapay.com). Only payouts initiated through the API send webhooks — payouts made from the dashboard do not.
...
```json
{
  ...
  "timestamp": "2026-07-26T19:02:15.000Z",
...
`destination.type` is `bank_account` for bank payouts and `mobile_money` for mobile money payouts (where `bank_code` is omitted). A failed payout is always reversed back to your wallet.
